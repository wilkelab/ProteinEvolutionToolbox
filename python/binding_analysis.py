...
def get_contacts_files(location, files, distance):
  ...
  for i in range(0,len(files)):
    ...
    receptor = structure.select(receptor_segnames).copy()
    ligand = structure.select(ligand_segnames).copy()
    contacts=receptor.select('within ' + distance + ' of ligand', ligand=ligand)
    num_contacts.append(len(contacts))

  os.chdir('..')

  return(np.mean(np.array(num_contacts)))
